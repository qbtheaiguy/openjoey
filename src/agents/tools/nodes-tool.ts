      ...
      try {
        switch (action) {
          ...
          case "run": {
            ...
            if (nodes.length === 0) {
              throw new Error(
                "system.run requires a paired companion app or node (no nodes available).",
              );
            }
            ...
            if (!supportsSystemRun) {
              throw new Error(
                "system.run requires a companion app or node; the selected node does not support system.run.",
              );
            }
