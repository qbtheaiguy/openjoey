...
/** Internal implementation used by OpenJoey router and by the public export when router is off. */
...
/** Public entry: when OpenJoey delivery router is enabled, delegates to router; otherwise uses internal impl. */
export async function deliverOutboundPayloads(
  params: Parameters<typeof deliverOutboundPayloadsInternal>[0],
): Promise<OutboundDeliveryResult[]> {
  const { isOpenJoeyDeliveryRouterEnabled } = await import("../../openjoey/config.js");
  if (isOpenJoeyDeliveryRouterEnabled(params.cfg)) {
    const { deliverViaRouter } = await import("../../openjoey/delivery/router.js");
    return deliverViaRouter(params);
  }
