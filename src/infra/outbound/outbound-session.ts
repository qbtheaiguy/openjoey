...
function resolveTelegramSession(
  ...
  const rawThreadId = normalizeThreadId(params.threadId);

  // Telegram topics are encoded in the peer id (chatId:topic:<id>) and must be numeric.
  const topicId =
    parsedThreadId ??
    (rawThreadId && /^\d+$/.test(rawThreadId) ? Number.parseInt(rawThreadId, 10) : undefined);
  ...
  const isChannel =
    chatType === "channel" || (chatType === "unknown" && params.resolvedTarget?.kind === "channel");
  const isGroup =
    chatType === "group" ||
    (chatType === "unknown" &&
      params.resolvedTarget?.kind &&
      params.resolvedTarget.kind === "group");
  const isMulti = isChannel || isGroup;

  const peerId = isMulti ? buildTelegramGroupPeerId(chatId, topicId) : chatId;
  const peerKind: RoutePeer["kind"] = isChannel ? "channel" : isGroup ? "group" : "dm";
  const peer: RoutePeer = {
    kind: peerKind,
    id: peerId,
  };
  ...
  // If we have a thread ID that wasn't consumed as a numeric topic, use it as a session suffix.
  const hasTopic = topicId != null;
  const sessionThreadId = hasTopic ? undefined : rawThreadId;
  ...
  return {
    ...
    from: isChannel
      ? `telegram:channel:${peerId}`
      : isGroup
        ? `telegram:group:${peerId}`
        : `telegram:${chatId}`,
    to: `telegram:${chatId}`,
    threadId: topicId ?? sessionThreadId,
  };
}
...
function resolveSlackSession(
  params: ResolveOutboundSessionRouteParams,
): OutboundSessionRoute | null {
  const stripped = stripProviderPrefix(params.target, "slack");
  const rawId = stripKindPrefix(stripped);
  if (!rawId) {
    return null;
  }

  const isExplicitChannel = /^channel:/i.test(stripped);
  const isExplicitGroup = /^group:/i.test(stripped);

  const isChannel = rawId.startsWith("C") || (isExplicitChannel && !rawId.startsWith("G"));
  const isGroup =
    params.resolvedTarget?.kind === "group" || rawId.startsWith("G") || isExplicitGroup;
  const isMulti = isChannel || isGroup;

  const peerKind: RoutePeer["kind"] = isGroup ? "group" : isChannel ? "channel" : "dm";
  const peer: RoutePeer = { kind: peerKind, id: rawId };
  const baseSessionKey = buildBaseSessionKey({
    cfg: params.cfg,
    agentId: params.agentId,
    channel: "slack",
  ...
  const chatType = peerKind === "dm" ? "direct" : peerKind;
  return {
    sessionKey: threadKeys.sessionKey,
    baseSessionKey,
    peer,
    chatType,
    from: isMulti ? `slack:${peerKind}:${rawId}` : `slack:${rawId}`,
    to: isMulti ? `${peerKind === "group" ? "group" : "channel"}:${rawId}` : `user:${rawId}`,
    threadId,
  };
}
...
export async function resolveOutboundSessionRoute(
  ...
  switch (params.channel) {
    ...
    case "slack":
      return resolveSlackSession({ ...params, target });
    default:
