...
- **OpenJoey repo — push guardrail:** This repo is the OpenJoey repo (origin = qbtheaiguy/openjoey, standalone/private). Before **any** `git push origin main`, **always** run `./scripts/openjoey-check-before-push.sh` to confirm which commits and files will be pushed; then run `git push origin main`. This keeps OpenJoey-specific fixes from being overwritten. See `docs/install/openjoey-sync-upstream.md`.
